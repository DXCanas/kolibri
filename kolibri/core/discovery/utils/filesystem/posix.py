...
def _try_to_get_drive_info_from_dbus(device):
    ...
    try:
        ...
        block_obj = bus.get_object('org.freedesktop.UDisks2', '/org/freedesktop/UDisks2/block_devices/' + block_device_name)
        ...
        return {
            "name": str(block.get("IdLabel") or " ".join([drive.get("Vendor"), drive.get("Model")]).strip()),
...
def _get_drivetype_from_dbus_drive_properties(drive_props):
    ...
    """
    if drive_props.get("ConnectionBus") == "sdio" or drive_props.get("Media") == "flash_sd":
        return drivetypes.SD_CARD
    elif drive_props.get("ConnectionBus") == "usb" or drive_props.get("Media") == "thumb":
...
def _try_to_get_drive_info_from_diskutil(device):
    ...
    diskutilp = subprocess.Popen("diskutil info {}".format(device), shell=True, stdout=subprocess.PIPE)
    diskutil_output, err = diskutilp.communicate()

    rows = [line.split(":", 1) for line in diskutil_output.decode().split("\n") if ":" in line]
    ...
    return {
        "drivetype": drivetype,
        "name": metadata.get("Volume Name") or metadata.get("Device / Media Name") or "",
